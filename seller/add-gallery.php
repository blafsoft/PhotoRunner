...
if(isset($_POST['update']))
{
	...
	{
		if($common->addgallery($_POST, $_FILES, true))
		{
			$common->redirect(APP_URL."seller/galleries.php");
		}
		else
...
<head>
	...
	.se-pre-con {
		...
		z-index: 9999;
		opacity: 0.5;
		background: url(<?php echo APP_URL; ?>images/loading6156.gif) center no-repeat #fff;
					...
					<div id="tabs-1">
						...
						<form  action=""  method="post" id="gallery-form" enctype='multipart/form-data'> 
							...
							<div>
								...
								<label for="file-7" id="tabfile-7" style="float:left;"><span></span> <strong><svg xmlns="http://www.w3.org/2000/svg" width="20" height="17" viewBox="0 0 20 17"><path d="M10 0l-5.2 4.9h3.3v5.1h3.8v-5.1h3.3l-5.2-4.9zm9.3 11.5l-3.2-2.1h-2l3.4 2.6h-3.5c-.1 0-.2.1-.2.1l-.8 2.3h-6l-.8-2.2c-.1-.1-.1-.2-.2-.2h-3.6l3.4-2.6h-2l-3.2 2.1c-.4.3-.7 1-.6 1.5l.6 3.1c.1.5.7.9 1.2.9h16.3c.6 0 1.1-.4 1.3-.9l.6-3.1c.1-.5-.2-1.2-.7-1.5z"/></svg> Choose a file&hellip;</strong></label> <?php if(isset($gellery->image) && !empty($gellery->image)) { ?><img src="<?php echo GALLERY_IMAGE . $gellery->image; ?>" style="float: left; height: 44px; margin-left: 10px;"><input type="hidden" name="oldimage" value="<?php echo $gellery->image; ?>" /> <?php } ?>
								<div style="clear:both">&nbsp;</div>
							</div>
							<div style="height:20px; clear:both"></div>
							<?php
